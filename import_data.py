...
def write_hdf5(origin_images, target_images, outfile):
    with h5py.File(outfile, "w") as f:
        ...
        f.create_dataset("target_images", data=target_images, dtype=target_images.dtype)
...
def pre_import_data():
    ...
    test_ratio = 0.10
    x_train, x_test, y_train, y_test = train_test_split(origin_images, target_images, test_size=1 - train_ratio)
    x_val, x_test, y_val, y_test = train_test_split(x_test, y_test, test_size=test_ratio/(test_ratio + validation_ratio))
    write_hdf5(x_train, y_train, 'train.hdf5')
    write_hdf5(x_val, y_val, 'val.hdf5')
    write_hdf5(x_test, y_test, 'test.hdf5')


def import_data(data_type):
    images = tfio.IODataset.from_hdf5(f'{data_type}.hdf5', dataset='/origin_images')
    labels = tfio.IODataset.from_hdf5(f'{data_type}.hdf5', dataset='/target_images')
    data = tf.data.Dataset.zip((images, labels)).batch(4, drop_remainder=True).prefetch(tf.data.experimental.AUTOTUNE)
    return data
